...
def generate_image_video(update, context, data_uri):
    ...
    try:
        resp = client.generations.create(
            model="gen3",
            prompt=prompt,
            image=data_uri,
            duration=duration,
        ...
        logger.info(f"Runway image-to-video response: {resp}")
        video_url = resp.output[0].url if resp.output else None
...
def _generate_text_video(update, context):
    ...
    try:
        ...
        logger.info(f"Runway text-to-video response: {resp}")
        video_url = resp.output[0].url if resp.output else None
        asyncio.run(send_video(update, video_url, context))
    except Exception as e:
        error_details = traceback.format_exc()
